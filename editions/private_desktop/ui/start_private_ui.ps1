...
function Set-ContentPolicy {
    ...
    if($UnknownToggle.IsChecked){$mask=$mask-bor 1024}
...
function Get-Quarantine {
    ...
    $restore=Join-Path $env:ProgramData "AIShield\quarantine\restore.jsonl";if(Test-Path $restore){Get-Content $restore|ForEach-Object{try{$r=$_|ConvertFrom-Json;if($latest.Contains([string]$r.id)){$restoreState=$(if($r.PSObject.Properties.Name-contains"state"){[string]$r.state}else{"committed"});$latest[[string]$r.id]|Add-Member NoteProperty state $(if($restoreState-eq"committed"){"released"}else{"committed"}) -Force}}catch{}}}
...
function Refresh-All {
    ...
    try{
        ...
        $UnknownToggle.IsChecked=($mask-band 1024)-ne0
...
foreach($toggle in @($DocumentsToggle,$ArchivesToggle,$ImagesToggle,$AudioToggle,$VideoToggle,$WebFilesToggle,$ProgramsToggle,$WindowsScriptsToggle,$DeveloperScriptsToggle,$LaunchersToggle,$UnknownToggle,$ReleaseRequiredToggle,$ScanFailureToggle)){$toggle.Add_Click({if($script:refreshing){return};try{Set-ContentPolicy;Set-Message "Dateityp-Richtlinie wurde aktiviert"}catch{Show-Error $_.Exception.Message;Refresh-All}})}
...
$ReleaseButton.Add_Click({$selected=$QuarantineGrid.SelectedItem;if($null-eq$selected-or$selected.State-ne"committed"){Set-Message "Bitte eine aktive Quarantänedatei auswählen";return};$save=[Windows.Forms.SaveFileDialog]::new();$save.Title="Ziel für freigegebene Datei";$save.FileName=[IO.Path]::GetFileName($selected.Source);if($save.ShowDialog()-eq[Windows.Forms.DialogResult]::OK){$reason=Read-Reason;if($reason){try{$broker=Get-ContentPolicyBroker;& $broker quarantine-restore $selected.Id $save.FileName $reason|Out-Null;if($LASTEXITCODE-ne0){throw "Freigabe fehlgeschlagen."};Set-Message "Datei wurde freigegeben und protokolliert";Refresh-All}catch{Show-Error $_.Exception.Message}}};$save.Dispose()})
